...
// CONFIDENCE LOGIC: This function calculates a retrieval score for each document chunk.
// While not a direct "confidence interval," this score (based on term frequency) determines
// which chunks are most relevant. Higher scores imply higher confidence that the chunk
// contains the answer.
...
const AnalyzeDocumentsOutputSchema = z.object({
  ...
  // CONFIDENCE LOGIC: This field is the output of the confidence calculation.
  // It is a scalar value (0-100) generated by the LLM, representing its self-assessed
  // certainty in the provided answer based on the source documents.
...
// CONFIDENCE LOGIC: The prompt explicitly asks the LLM to output a `confidence` field (0-100).
// This relies on the model's internal self-consistency and calibration.
// While not a statistical confidence interval, it serves as a confidence score for the answer.
